...
which(stress<10)[1]
```
The number of dimensions necessary to obtain a good representation is 22 dimensions.
...
which(stress==min(stress))
which(stress==max(stress))
best_run<-points_100[[which(stress==min(stress))]]
worst_run<-points_100[[which(stress==max(stress))]]
...
```{r}
corr_matrix<-cbind(cmd.chr21$points,maximum_points[,1:2])
round(cor(corr_matrix),digits=2)
```
